...
| Armchair | `armchair.glb` | [Leather Armchair](https://sketchfab.com/3d-models/leather-armchair-gamedev-b73a5f3c67294f8d8f4f8d7a6e8b9c0d) | Free |
...
| Floor Lamp | `floor_lamp.glb` | [Antique Floor Lamp](https://sketchfab.com/variandev/collections/antique-69793c77765148bda1aa632929fb2320) (from collection) | Free |
...
1. **[Victorian and Vintage Furniture](https://sketchfab.com/apatel/collections/dl-victorian-and-vintage-furniture-85abfb5d444e48aeb3e472db46502b4b)** by Antoine Patel
   - Curated collection of downloadable Victorian furniture

2. **[Antique Collection](https://sketchfab.com/variandev/collections/antique-69793c77765148bda1aa632929fb2320)** by VarianDev
   - Includes: Leather armchair, antique desk, bookshelf, floor lamp, chandelier, wardrobe
...
3. Select **"glTF"** format (or "Original" if glTF not available)
4. Extract the ZIP file
5. Find the `.glb` or `.gltf` file
6. Rename to match expected filename (e.g., `bookshelf.glb`)
...
- Keep total models under 50MB for fast loading
- Use GLB format (binary, smaller than GLTF)
- If model is too large, use Blender to:
...
If a model appears too big/small or in the wrong position, edit `MODEL_CONFIG` in `room.html`:

```javascript
const MODEL_CONFIG = {
    bookshelf: {
        file: 'bookshelf.glb',
        position: [-3.5, 0, -2.5],  // [x, y, z]
        rotation: [0, Math.PI/2, 0], // [x, y, z] in radians
        scale: 1                      // uniform scale, or [x, y, z]
    },
    // ... etc
};
```
...
- Verify file is in `assets/models/` folder
- Verify filename matches `MODEL_CONFIG`

**Model is wrong size:**
- Adjust `scale` in `MODEL_CONFIG` (try 0.01 for very large models)

**Model is rotated wrong:**
- Adjust `rotation` values (use Math.PI for 180°, Math.PI/2 for 90°)

**Textures missing:**
- Re-download as GLB format (embeds textures)
- Or place texture files in same folder as .gltf file
